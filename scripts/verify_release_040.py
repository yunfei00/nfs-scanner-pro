#!/usr/bin/env python3
"""Release_040 自动验收 — Real Spectrum Single Point Amplitude Read。"""
...
def check_real_spectrum_imports(report: verification_report.VerificationReport) -> None:
    ...
    try:
        ...
        ok = (
            ...
            and hasattr(SpectrumScpiAdapter, "read_single_point_amplitude")
        )
        if ok:
...
def check_default_real_hardware_disabled(report: verification_report.VerificationReport) -> None:
    ...
    try:
        ...
        single = adapter.read_single_point_amplitude()
        ...
        ok = (
            ...
            and single.get("disabled") is True
        ...
        if ok:
            ...
            report.fail_check(
                ...
                f"connect={connect_msg!r} single={single} all={all_safe}",
            )
    except Exception as exc:  # noqa: BLE001
...
def check_amplitude_response_parser(report: verification_report.VerificationReport) -> None:
    report.start_check("amplitude_response_parser")
    try:
        ...
        for raw, expected in cases:
            parsed = SpectrumScpiAdapter.parse_amplitude_response(raw)
            if not parsed.get("ok") or parsed.get("amplitude") != expected:
                bad.append(f"{raw!r} -> {parsed}")
        for raw in bad_cases:
            parsed = SpectrumScpiAdapter.parse_amplitude_response(raw)
            if parsed.get("ok"):
                bad.append(f"{raw!r} should fail")
        if bad:
            report.fail_check("amplitude_response_parser", "; ".join(bad))
        else:
            report.pass_check("amplitude_response_parser")
    except Exception as exc:  # noqa: BLE001
        report.fail_check("amplitude_response_parser", str(exc))


def check_marker_response_parser(report: verification_report.VerificationReport) -> None:
    report.start_check("marker_response_parser")
    try:
        from nfs_scanner_pro.devices.real.spectrum_scpi_adapter import SpectrumScpiAdapter

        amp = SpectrumScpiAdapter.parse_marker_response("-23.45")
        amp_phase = SpectrumScpiAdapter.parse_marker_response("-23.45,112.3")
        freq_amp = SpectrumScpiAdapter.parse_marker_response("2.45E9,-23.45")
        triple = SpectrumScpiAdapter.parse_marker_response("-23.45,0,0")
        bad = SpectrumScpiAdapter.parse_marker_response("not-a-number")

        ok = (
            amp.get("ok")
            and amp.get("amplitude") == -23.45
            and amp_phase.get("ok")
            and amp_phase.get("amplitude") == -23.45
            and freq_amp.get("ok")
            and abs(freq_amp.get("frequency_hz", 0) - 2.45e9) < 1
            and freq_amp.get("amplitude") == -23.45
            and triple.get("ok")
            and triple.get("amplitude") == -23.45
            and not bad.get("ok")
        )
        if ok:
            report.pass_check("marker_response_parser")
        else:
            report.fail_check(
                "marker_response_parser",
                f"amp={amp} phase={amp_phase} fa={freq_amp} triple={triple} bad={bad}",
            )
    except Exception as exc:  # noqa: BLE001
        report.fail_check("marker_response_parser", str(exc))


def check_scpi_marker_query_whitelist(report: verification_report.VerificationReport) -> None:
    report.start_check("scpi_marker_query_whitelist")
    try:
        from nfs_scanner_pro.devices.real.spectrum_scpi_adapter import SpectrumScpiAdapter

        allowed_ok = all(
            SpectrumScpiAdapter.validate_query_command(cmd)[0]
            for cmd in ALLOWED_MARKER_SCPI
        )
        forbidden_ok = all(
            not SpectrumScpiAdapter.validate_query_command(cmd)[0]
            for cmd in FORBIDDEN_MARKER_SCPI
        )
        if allowed_ok and forbidden_ok:
            report.pass_check("scpi_marker_query_whitelist")
        else:
            report.fail_check(
                "scpi_marker_query_whitelist",
                f"allowed={allowed_ok} forbidden={forbidden_ok}",
            )
    except Exception as exc:  # noqa: BLE001
        report.fail_check("scpi_marker_query_whitelist", str(exc))
...
def check_no_connection_when_disabled(report: verification_report.VerificationReport) -> None:
    ...
    try:
        ...
        with mock.patch.object(
            ...
            single = adapter.read_single_point_amplitude()
        ok = (
            single.get("disabled") is True
            and adapter._socket is None
            and "未启用" in str(single.get("error", ""))
        )
        if ok:
            report.pass_check("no_connection_when_disabled")
        else:
            report.fail_check("no_connection_when_disabled", str(single))
    except Exception as exc:  # noqa: BLE001
...
def check_real_device_manager_single_point_status(
    report: verification_report.VerificationReport,
) -> None:
    report.start_check("real_device_manager_single_point_status")
    env_backup = os.environ.pop("NFS_ENABLE_REAL_HARDWARE", None)
    try:
        ...
        single = spectrum_snap.get("single_point", {})
        ok = (
        ...
        if ok:
            report.pass_check("real_device_manager_single_point_status")
        else:
            report.fail_check(
                "real_device_manager_single_point_status",
                f"single={single} connect={connect}",
            )
    except Exception as exc:  # noqa: BLE001
        report.fail_check("real_device_manager_single_point_status", str(exc))
    finally:
        if env_backup is not None:
            os.environ["NFS_ENABLE_REAL_HARDWARE"] = env_backup


def check_check_spectrum_single_point_safe_default(
    report: verification_report.VerificationReport,
) -> None:
    report.start_check("check_spectrum_single_point_safe_default")
    env = os.environ.copy()
    env.pop("NFS_ENABLE_REAL_HARDWARE", None)
    proc = subprocess.run(
        [sys.executable, str(SCRIPTS_DIR / "check_spectrum_single_point_safe.py")],
        cwd=str(ROOT),
        capture_output=True,
        text=True,
        encoding="utf-8",
    ...
    if ok:
        report.pass_check("check_spectrum_single_point_safe_default")
    else:
        report.fail_check("check_spectrum_single_point_safe_default", text[-400:])
...
def check_source_scpi_command_safety(report: verification_report.VerificationReport) -> None:
    ...
    try:
        ...
        ok = (
            ...
            and "MARKER_AMPLITUDE_COMMANDS" in text
            and "validate_query_command" in text
            and "is_real_hardware_enabled()" in text
            and "read_marker_amplitude" in text
            and 'self._transport_query("INIT' not in text
        )
        transport_bad = (
            'self._transport_query("CALC:DATA?' in text
            or 'self._execute_query("INIT' in text
        )
        ok = ok and not transport_bad
        if ok:
            report.pass_check("source_scpi_command_safety")
        else:
            report.fail_check("source_scpi_command_safety", "missing guard or unsafe transport")
    except Exception as exc:  # noqa: BLE001
...
def write_acceptance_report(report: verification_report.VerificationReport) -> Path:
    out = (
        ROOT
        / "docs/product-spec/release/Release_040_Real_Spectrum_Single_Point_Amplitude_Read/ACCEPTANCE_REPORT.md"
    )
...
def main() -> int:
    setup_path()
    report = verification_report.VerificationReport("040")

    check_compileall(report)
    check_real_spectrum_imports(report)
    check_default_real_hardware_disabled(report)
    check_amplitude_response_parser(report)
    check_marker_response_parser(report)
    check_scpi_marker_query_whitelist(report)
    check_no_connection_when_disabled(report)
    check_real_device_manager_single_point_status(report)
    check_check_spectrum_single_point_safe_default(report)
