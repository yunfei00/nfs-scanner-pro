...
将 **Release 010 + Release 010.5 高保真设计** 落地为可运行的 **PySide6 四页主窗口 Mock 原型**，用于视觉对齐与后续业务开发前的 UI 壳层验收。
...
- Release 010.5 四页补全：[Release_010_5_High_Fidelity_Completion/README.md](../Release_010_5_High_Fidelity_Completion/README.md)
- ADR-0022 / ADR-0023
...
- 菜单栏 / 工具栏 / 设备状态栏 / 底部状态栏
- 左侧 64px 图标导航，Hover 180px 展开；**仅** 扫描 / 设备 / 分析 / 报告
- **四页** QStackedWidget：扫描、设备、分析、报告
- 中央 QGraphicsView PCB 画布 Mock（整图热力图 QPixmap 叠加）
- 色带、小地图、坐标浮窗、扫描区域手柄 Mock（扫描页）
- 设备页四卡片 Mock（运动平台 / 频谱仪 / 相机 / 舵机）
- 分析页 PCB + 热力图 + 光标读数浮窗
- 报告页列表 + PDF 风预览 + 报告设置 Dock
- 右侧 QDockWidget 按页切换：扫描参数 / 设备配置 / 分析参数 / 报告设置
- 视图菜单「显示参数面板」控制**当前页** Dock 显隐
- 工具栏按页切换（扫描类 vs 报告类）
...
src/nfs_scanner_pro/
  ...
  ui/
    ...
    device_config_dock.py
    analysis_parameter_dock.py
    report_settings_dock.py
    mock_data.py
    pages/
      scan_page.py
      device_page.py
...
| 5 | 项目入口 | 仅在「文件」菜单 |
| 6 | 设备状态栏 | 单行四设备 + 元数据 |
| 7 | 参数 Dock | QDockWidget，按页切换 |
| 8 | 画布 | QGraphicsView + 整图热力图 |
| 9 | 默认隐藏 | 日志/频谱/统计不默认显示 |
| 10 | 视图菜单 | 「显示参数面板」可切换当前页 Dock |
| 11 | 四页切换 | 导航 + 工具栏 + 状态栏联动 |
| 12 | Mock only | 无真实设备代码 |
| 13 | QSS | dark_theme.qss 生效 |
| 14 | 视觉 | 接近 Release 010 / 010.5 高保真 |
...
**依赖**：Release 008、009.5、009.8、009.9、010、010.5  
