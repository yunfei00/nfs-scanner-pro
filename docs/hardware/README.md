...
1. 运行联调准备：`python scripts/validate_commissioning_readiness.py`
2. 运行接口审计：`python scripts/check_hardware_interface_inventory.py`
3. 生成诊断报告：`python scripts/generate_hardware_bringup_report.py`
4. **Commissioning**：`python scripts/start_hardware_commissioning.py --mode offline` → `--mode fake`
5. 复制 `config/hardware.local.example.yaml` → `config/hardware.local.yaml`
6. 逐项接设备：运动 status → 频谱 IDN → 相机 list → 舵机 state
7. UI 真实扫描控制台：dry-run → fake-run
8. 全部子开关确认后再 real-run（门禁见 commissioning/10-real-run-gate.md）
...
- [commissioning/README.md](commissioning/README.md) — Release_048 现场联调流程
